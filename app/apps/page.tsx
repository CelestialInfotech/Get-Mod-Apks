...
import { useApksByFilter } from "@/hooks/use-apks"

export default function AppsPage() {
  const [currentPage, setCurrentPage] = useState(1)
  const limit = 20

  const { apks: apps, loading, error, data } = useApksByFilter("apps", currentPage, limit)
    ...
    <div className="min-h-screen bg-background text-foreground transition-colors duration-300">
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        {loading ? (
          ...
          <div className="text-center py-12 text-muted-foreground">No apps available</div>
        )}
      </main>
    </div>
  )
